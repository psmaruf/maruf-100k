  ...
  onStart: async ({ event, api, args, message }) => {
    try {
      ...
      try {
        ...
        console.log("Could not set reaction:", reactionError.message);
      }
      ...
      const filteredVideos = Array.isArray(searchResponse.data) 
        ? searchResponse.data.filter((video) => {
            try {
              return parseDuration(video?.time) < 600; // 10 minutes
            } catch {
              return false;
            }
          })
      ...
      const styledMessage = `🎧 Title   : ${selectedVideo.title || "Unknown"}
⏱ Duration : ${selectedVideo.time || "Unknown"}
...
By —͟͟͞͞💜َ PS MARUF `;
      ...
      try {
        ...
        console.log("Could not set error reaction:", reactionError.message);
      }
...
try {
  ...
  console.error("Wrapper initialization error:", wrapperError.message);
}
